...
import { API_ENDPOINT, REDIRECT_ENDPOINT } from '@/config';
...
const shortenLink = async () => {
  try {
    // Make API call to shorten link and return its short link identifer.
    const response = await axios.post(`${API_ENDPOINT}/shorten?originalLink=${originalLink.value}`);
    
    // Set shortened link using redirect endpoint (e.g, 'https://strwb.cc/{shortLink}').
    shortenedLink.value = `${REDIRECT_ENDPOINT}/${response.data.shortLink}`;
